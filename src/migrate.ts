...
class DefaultValues {
    @repository(SimpleUserRepository) public userRepository: SimpleUserRepository
    ...
    private async saveRoles(): Promise<void> {
        ...
        if (!await this.roleRepository.exists(Roles.Admin.ID)) {
            const admin: Role = new Role({ id: Roles.Admin.ID, name: Roles.Admin.NAME })
        ...
        if (!await this.roleRepository.exists(Roles.Medico.ID)) {
            const medico: Role = new Role({ id: Roles.Medico.ID, name: Roles.Medico.NAME })
    ...
    private async saveUser(): Promise<void> {
        const users: Count = await this.userRepository.count({ roleId: Roles.Admin.ID })
        if (users.count < 1) {
